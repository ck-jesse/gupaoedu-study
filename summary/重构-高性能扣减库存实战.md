...
public class OrderIdCacheService implements CacheService<StockListDTO.StockDTO, Integer> {
    ...
    private static final String CACHE_PREFIX = "goods:spec:orderId:";
    private static final String SPLIT = "_";

    @Autowired
    RedissonClient redissonClient;

    private String buildCacheName(Integer goodsSpecId) {
        return CACHE_PREFIX + goodsSpecId;
    }

    @Override
    public String buildCacheKey(StockListDTO.StockDTO stockDTO) {
        return stockDTO.getOrderId() + SPLIT + stockDTO.getStockOperate();
    }

    @Override
    public Integer get(StockListDTO.StockDTO stockDTO) {
        RMapCache<String, Integer> mapCache = redissonClient.getMapCache(this.buildCacheName(stockDTO.getGoodsSpecId()));
        return mapCache.get(this.buildCacheKey(stockDTO));
    }
    ...
    public Integer put(StockListDTO.StockDTO stockDTO, Integer value) {
        RMapCache<String, Integer> mapCache = redissonClient.getMapCache(this.buildCacheName(stockDTO.getGoodsSpecId()));
        mapCache.fastPut(this.buildCacheKey(stockDTO), value);
        log.debug("[put][orderId] cacheName={}, key={}, value={}", this.buildCacheName(stockDTO.getGoodsSpecId()), this.buildCacheKey(stockDTO), value);
        return value;
    }

    /**
     * true 表示put成功（缓存项不存在），false 表示put失败（缓存项存在）
     */
    @Override
    public boolean putIfAbsent(StockListDTO.StockDTO stockDTO, Integer value) {
        RMapCache<String, Integer> mapCache = redissonClient.getMapCache(this.buildCacheName(stockDTO.getGoodsSpecId()));
        boolean rslt = mapCache.fastPutIfAbsent(this.buildCacheKey(stockDTO), value);
        log.debug("[putIfAbsent][orderId] cacheName={}, key={}, value={}, rslt={}", this.buildCacheName(stockDTO.getGoodsSpecId()), this.buildCacheKey(stockDTO), value, rslt);
        return rslt;
    }
    ...
    public void evict(StockListDTO.StockDTO stockDTO) {
        RMapCache<String, Integer> mapCache = redissonClient.getMapCache(this.buildCacheName(stockDTO.getGoodsSpecId()));
        long rslt = mapCache.fastRemove(this.buildCacheKey(stockDTO));
        log.info("[evict][orderId] cacheName={}, key={}, result={}", this.buildCacheName(stockDTO.getGoodsSpecId()), this.buildCacheKey(stockDTO), rslt);
    }

    @Override
    public boolean isExists(StockListDTO.StockDTO stockDTO) {
        RMapCache<String, Integer> mapCache = redissonClient.getMapCache(this.buildCacheName(stockDTO.getGoodsSpecId()));
        return mapCache.containsKey(this.buildCacheKey(stockDTO));
    }
```
