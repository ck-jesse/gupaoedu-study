 ...
 * <p>
 * 实际使用过的场景：
 * 1、开放平台Openapi应用中添加不同的过滤器（如：参数验证过滤器、签名过滤器、验签过滤器、IP白名单过滤器、权限控制过滤器、流控过滤器、时间戳校验过滤器、检查提交数据大小过滤器等）
 * 2、针对第三方api的基础封装可以采用责任链模式（如：参数验证过滤器、签名过滤器、验签过滤器、流控过滤器等）
 * <p>
...
public class Test {
    ...
    public void test1() {
        ...
        // 为了让审批流程更加灵活并实现采购单的链式传递和处理，我们现在使用职责链模式来实现采购单的分级审批
